...
<ul class="nav-main " id="nav-bar-land">
                    ...
                    @if ($menu->childs && $menu->childs->count() > 0)
                        @foreach ($menu->childs as $c_menu)
                            @if ($c_menu->checkMenuPermission($c_menu->menu_url))
                                <li>
                                    <a class="{{ request()->is($c_menu->menu_url) ? ' active' : '' }}"
                                        href="{{ url($c_menu->menu_url) }}">
                                        {{ $c_menu->name }}
                                    </a>
                                </li>
                            @endif
    ...
    <li class="{{ request()->is('admin/*') ? ' open' : '' }}">
        ...
        <ul>
            ...
            <li>
                <a class="{{ request()->is('admin/user-roles') ? ' active' : '' }}"
                    href="{{ route('admin.user-roles.index') }}">Role Management</a>
            </li>
            <li>
                <a class="{{ request()->is('admin/menus/index') ? ' active' : '' }}"
                    href="{{ route('admin.menus.index') }}">Menu Management</a>
            </li>
            <li>
                <a class="nav-submenu" data-toggle="nav-submenu" href="#"><span
                        class="sidebar-mini-hide">Permission Management</span></a>
                <ul>
                    <li>
                        <a class="{{ request()->is('admin/menu-role-permission') ? ' active' : '' }}"
                            href="{{ url('admin/menu-role-permission') }}">Menu Permission</a>
                    </li>
                    <li>
                        <a class="{{ request()->is('admin/permissions') ? ' active' : '' }}"
                            href="{{ url('admin/permissions') }}">Permission List</a>
                    </li>
                </ul>
            </li>
            <li>
                <a class="{{ request()->is('admin/sms-configuration') ? ' active' : '' }}"
                    href="{{ url('admin/sms-configuration') }}">SMS Configuration</a>
            </li>
        </ul>
    </li>
